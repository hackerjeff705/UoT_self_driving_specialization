...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K_k = p_cov_check @ h_jac.T @ np.linalg.inv((h_jac @ p_cov_check @ h_jac.T) + r_cov)

    # 3.2 Compute error state
    delta_x_k = K_k @ (y_k - p_check)

    # 3.3 Correct predicted state
    p_hat = p_check + delta_x_k[:3]
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    Cns_fprev = Cns @ imu_f.data[k - 1]

    # Position
    p_est[k] = p_est[k - 1] + delta_t * v_est[k - 1] + 0.5 * (delta_t ** 2) * (Cns_fprev + g)

    # Velocity
    v_est[k] = v_est[k - 1] + delta_t * (Cns_fprev + g)

    # Orientation
    q_wt = Quaternion(axis_angle=imu_w.data[k - 1] * delta_t)
    q_est[k] = q_wt.quat_mult_right(q_est[k-1])

    # 1.1 Linearize the motion model and compute Jacobians
    F_prev = np.eye(9)
    F_prev[:3, 3:6] = np.eye(3) * delta_t
    F_prev[3:6, 6:] = -skew_symmetric(Cns_fprev) * delta_t

    # 2. Propagate uncertainty
    Q_prev = np.identity(6)
    Q_prev[:3, :3] *= delta_t**2 * var_imu_f
    Q_prev[3:, 3:] *= delta_t**2 * var_imu_w

    p_cov[k] = F_prev @ p_cov[k-1] @ F_prev.T + l_jac @ Q_prev @ l_jac.T
