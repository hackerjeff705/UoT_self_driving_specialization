...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            new_goal_index, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)
            self._goal_index = new_goal_index if stop_sign_found else goal_index
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            wp_index += 1
            x0, y0, _ = waypoints[wp_index-1]
            x1, y1, _ = waypoints[wp_index]
            arc_length += math.sqrt((x0 - x1)**2 + (y0 - y1)**2)
            if arc_length >= self._lookahead:
                break
        # ------------------------------------------------------------------
        return wp_index
...
def get_closest_index(waypoints, ego_state):
    ...
    # print(f"wp_arr: {wp_arr[:, :2]}")
    # es_arr = np.array(ego_state)
    # print(f"es_arr: {es_arr[:2]}")
    # print(f"euclidean: {np.min(np.sqrt(np.sum((es_arr[:2] - wp_arr[:, :2])**2, axis=1)))}")
    # closest_len = np.min(np.sqrt(np.sum((es_arr[:2] - wp_arr[:, :2])**2, axis=1)))
    # closest_index = np.where(closest_len)
    # print(f"closest lenght: {closest_len}, index: {closest_index[0]}")
    # print("-------------------------------------")
